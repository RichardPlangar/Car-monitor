const { getLink, getInfoFromAdvertisement } = require('./routes');
const {
  insertIntoBasicsTable,
  insertIntoCarConditionTable,
} = require('./sqlQuery');
const {
  formatAdvertiseId,
  getCarCondition,
  formatText,
} = require('./formatData');

async function getAllAdvertisement(pages) {
  for (let i = 1; i <= pages; i++) {
    const allLinks = await getLink(
      'https://www.hasznaltauto.',
      i
    );
    const formattedAdvertiseIds = await formatAdvertiseId(allLinks);
    insertIntoBasicsTable(formattedAdvertiseIds);
  }
}

async function insertAllCarCondition(numberOfCars) {
  const allCarCondition = await getCarCondition(numberOfCars);
  insertIntoCarConditionTable(allCarCondition);
}
